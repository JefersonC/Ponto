...
-- Schema ponto
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `ponto` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
USE `ponto` ;
...
CREATE TABLE IF NOT EXISTS `ponto`.`cargo` (
  ...
  `cargaHoraria` INT NOT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `ponto`.`usuario` (
  ...
  `identificacao` VARCHAR(20) NULL COMMENT '',
  `rfid` VARCHAR(16) NULL COMMENT '',
  `dataCadastro` DATETIME NULL DEFAULT CURRENT_TIMESTAMP COMMENT '',
  `dataAdmissao` DATETIME NULL COMMENT '',
  `estado` INT(1) NOT NULL DEFAULT 1 COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  UNIQUE INDEX `idxRfid` (`rfid`(16) ASC)  COMMENT '',
  INDEX `fkUsuarioCargo_idx` (`idCargo` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `ponto`.`ponto` (
  ...
  `estado` INT(1) NOT NULL DEFAULT 1 COMMENT '',
  `dataAbertura` DATETIME NOT NULL COMMENT '',
  `dataFechamento` DATETIME NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  INDEX `fkPontoUsuario_idx` (`idUsuario` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `ponto`.`sistema` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `versao` DOUBLE(4,2) NOT NULL DEFAULT 0 COMMENT '',
  `cargaHoraria` INT NOT NULL DEFAULT 160 COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '')
ENGINE = InnoDB;
